...
### Support/DiscordNotifier

Centralized Discord webhook sender. All Discord notifications route through this class.

- `notifyContactSubmission(string $name, string $email, string $message): void` — contact form embed
- `sendEmbed(array $embed): void` — generic embed sender (used by all event subscribers)

**Constructor:** `DiscordNotifier(HttpClientInterface $httpClient, string $webhookUrl, LoggerInterface $logger)`

Empty `$webhookUrl` disables notifications silently. Failed requests are logged, never thrown.

### External HTTP Pattern

All external HTTP calls use `Waaseyaa\HttpClient\HttpClientInterface` (from `waaseyaa/http-client` package). No raw `stream_context_create()` or `file_get_contents()`.

| Service | Class | Method |
|---------|-------|--------|
| Discord webhooks | `DiscordNotifier` | `HttpClientInterface->post()` |
| Anthropic API | `QualificationService` | `HttpClientInterface->post()` |
| North-cloud search | `RfpImportService` | `HttpClientInterface->post()` |

Providers create `StreamHttpClient` instances: default 30s timeout for API calls, 5s timeout for Discord webhooks.

### Event Subscribers

- **LeadCreatedSubscriber:** Discord notification via `DiscordNotifier::sendEmbed()`, activity logging
